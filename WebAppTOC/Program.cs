...
app.MapGet("/wallets", async (ApplicationDbContext db) => await db.Wallets.ToListAsync())
    .WithName("GetWallets")
    .WithOpenApi();
...
app.MapGet(
        "/wallets/{id}",
        async (ApplicationDbContext db, int id) =>
        {
...
app.MapPost(
        "/wallets",
        async (ApplicationDbContext db, WalletCreateDto walletDto) =>
        {
...
app.MapGet(
        ...
        async (ApplicationDbContext db, int walletId) =>
        {
            ...
            if (transactions == null || !transactions.Any())
                return Results.NotFound(
                    $"No se encontraron transacciones para la billetera con ID {walletId}"
                );
...
app.MapPost(
        "/wallets/{walletId}/transactions",
        async (ApplicationDbContext db, int walletId, Transaction transaction) =>
        {
            var wallet = await db.Wallets.FindAsync(walletId);
            if (wallet == null)
                return Results.NotFound($"No se encontró la billetera con ID {walletId}");

            using var transactionDb = await db.Database.BeginTransactionAsync();
            try
            {
                ...
                return Results.Created(
                    $"/wallets/{walletId}/transactions/{transaction.Id}",
                    transaction
                );
            }
            catch
...
app.MapPut(
        "/wallets/{id}",
        async (ApplicationDbContext db, int id, WalletUpdateDto walletDto) =>
        {
            var wallet = await db.Wallets.FindAsync(id);
